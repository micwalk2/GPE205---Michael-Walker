...
public class AIController : Controller
{
    ...
    public bool CanSee(GameObject target)
    {
        ...
        Debug.Log("AI forward direction: " + pawn.transform.forward);
        Debug.Log("Vector to target: " + AIToTargetVector);

        // If the angle is not in our field of view, we cannot see the target
        if (angleToTarget >= fieldOfView)
        {
            Debug.Log("Outside of field of view.  angleToTarget " + angleToTarget + "fieldofView " + fieldOfView);
            return false;
        }
        ...
        if (!Physics.Raycast(pawn.transform.position, AIToTargetVector, out hit, visionDistance))
        {
